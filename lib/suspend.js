...
var suspend = module.exports = function fn(generator) {
	...
	return aritizeParameterlessFn(function() {
 ...
 */
 suspend.callback = function callback(generator) {
	...
	// Arity is (gen.length + 1) because it's generator args + callback arg
	}, generator.length + 1);
};
 ...
 */
suspend.run = function run(generator, callback, thisArg) {
	if (!isGeneratorFunction(generator)) {
...
function aritizeParameterlessFn(fn, arity) {
	// If arity == 0, or the function isn't parameterless, do nothing
	if((!arity) || (!!fn.length)) return fn;

	var aritized;
	
	// Use eval to create the function as it closures 'fn' - unlike Function ctor
	eval('aritized = function(' + Array.apply(Array, Array(arity))
		.map(function(e, i) { return '_' + i; })
		.join() + '){' +
